...
export async function POST(request: NextRequest) {
  try {
    ...
    const textContent = generateFormContent(formData, '0-2');

    if (action === 'email') {
      // For email, return the content as JSON
      // In production, you would send an actual email with PDF attachment
      ...
      // Return as downloadable text file (can be converted to PDF client-side)
      return new NextResponse(textContent, {
        headers: {
          'Content-Type': 'text/plain',
          'Content-Disposition': `attachment; filename="0-2-USTEPS-Access-Form-${formData.lastName || 'form'}.txt"`,
        },
      });
    }
  } catch (error) {
...
function generateFormContent(formData: any, formType: string): string {
  return `
================================================================================
FORM ${formType}: DHHS PRIVATE SUPPORT COORDINATOR USTEPS ACCESS FORM
================================================================================
