...
export default function Form02Page() {
  ...
  const handleSubmit = async (e: React.FormEvent, action: 'download' | 'email') => {
    ...
    try {
      const response = await fetch('/api/forms/0-2', {
        method: 'POST',
      ...
      if (response.ok) {
        if (action === 'download') {
          ...
          a.href = url;
          a.download = `0-2-USTEPS-Access-Form-${formData.lastName || 'form'}-${Date.now()}.txt`;
        ...
        <Card className="border border-gray-200 shadow-sm">
          <CardHeader>
            <CardTitle className="text-2xl font-semibold text-black">
              Form 0-2: DHHS Private Support Coordinator USTEPS Access Form
            </CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
              ...
              {/* USTEPS Access Information */}
              <div>
                <h3 className="text-lg font-semibold text-black mb-4 pb-2 border-b">USTEPS Access Information</h3>
                <div className="grid gap-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg text-black bg-white"
                      placeholder="Describe why you need USTEPS access..."
                    />
                  </div>
